...
public class FieldBuilderTests
{
    ...
    [InlineData("with-metadata-only")]
    [InlineData("with-arguments-and-metadata")]
    [InlineData("metadata-merging")]
    public void FieldBuilder_WithMetadata_Comprehensive(string scenario)
    {
        var scenarios = new TestScenarioBag<FieldDefinition>()
            ...
            .Register("with-arguments-and-metadata",
                arrange: () =>
                {
                    var args = new Dictionary<string, object?> { { "limit", 10 } };
                    var meta = new Dictionary<string, object?> { { "indexed", true } };
                    return FieldBuilder.Create([], "query")
    ...
    [Fact]
    public void FieldBuilder_AddField_EmptyFieldName_ThrowsArgumentException()
    {
        var builder = FieldBuilder.Create([], "root");
        var act = () => builder.AddField("");
        act.Should().Throw<ArgumentException>().WithParameterName("fieldName");
    }

    [Theory]
    [InlineData("dict_merge_both_dicts", "key1", "key2")]
    [InlineData("dict_merge_nested", "nested", "deep")]
    public void FieldBuilder_Where_WithDictionaryValues_MergesRecursively(string _, string key1, string key2)
    {
        var builder = FieldBuilder.Create([], "root");
        var dict1 = new Dictionary<string, object?> { [key1] = "value1" };
        var dict2 = new Dictionary<string, object?> { [key2] = "value2" };

        builder.Where("filter", dict1);
        builder.Where("filter", dict2);

        var result = builder.Build();
        result.Arguments.Should().ContainKey("filter");
        var mergedDict = result.Arguments["filter"] as IDictionary<string, object?>;
        mergedDict.Should().NotBeNull();
        mergedDict.Should().ContainKey(key1);
        mergedDict.Should().ContainKey(key2);
    }

    [Theory]
    [InlineData("override_with_string", "initial")]
    [InlineData("override_with_number", 42)]
    public void FieldBuilder_Where_WithNonDictionaryValues_OverridesValue(string _, object initialValue)
    {
        var builder = FieldBuilder.Create([], "root");
        builder.Where("key", initialValue);
        builder.Where("key", "new_value");

        var result = builder.Build();
        result.Arguments["key"].Should().Be("new_value");
    }
