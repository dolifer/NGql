...
if (args.Length < 2)
{
    Console.Error.WriteLine("Usage: BenchmarkMerger <local-artifacts-dir> <published-artifacts-dir>");
    return 1;
}
...
if (localCsvs.Length == 0) { Console.Error.WriteLine($"No CSV report files found under: {args[0]}"); return 1; }
if (publishedCsvs.Length == 0) { Console.Error.WriteLine($"No CSV report files found under: {args[1]}"); return 1; }
...
static string? ComputeRatio(string current, string? baseline)
{
    ...
    if (c is null || b is null || b == 0) return null;
    var ratio = c.Value / b.Value;
    var marker = ratio < 0.95 ? " 🟢" : ratio > 1.05 ? " 🔴" : "";
