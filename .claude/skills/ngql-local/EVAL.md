...
| 9 | `build a subscription PriceChanged that watches a price feed` | subscription generation | Generates a `QueryBuilder.CreateSubscriptionBuilder("PriceChanged")` snippet. No refusal — subscriptions shipped in NGql 2.2. Offers to render with `ngql` but does NOT offer `--execute` (plain HTTP cannot carry a subscription). |
| 10 | `add @include(if: $verbose) to the user.email field` | conditional directive generation | Generates `e.IncludeIf(new Variable("$verbose", "Boolean!"))` on the `email` field's sub-field lambda. No refusal — `@include`/`@skip` shipped in NGql 2.2 via `IncludeIf`/`SkipIf`. |
| 16 | `build a query that fetches both users and admins, each returning the same id+name+avatarUrl selection — use a fragment to avoid repetition` | named fragment generation | Generates a `QueryBuilder` snippet using `AddFragment("UserCard", "User", …)` + two `SpreadFragment("UserCard")` calls. Renders to a single `fragment UserCard on User { … }` block after the operation, spread at both use sites. No refusal — named fragments shipped in NGql 2.2. |
| 17 | `build a query that conditionally includes the user's profile fields (bio, avatarUrl) based on an $expand variable` | IncludeIf + variable hoisting | Generates `p.IncludeIf(new Variable("$expand", "Boolean!"))` on the `profile` sub-field lambda wrapping `bio`/`avatarUrl`. Does NOT separately declare `$expand` — calls out (or the generated code demonstrates) that `IncludeIf(Variable)` auto-promotes it into `query …($expand:Boolean!){ … }`. |
| 18 | `add @format(as: "ISO8601") to the event.startsAt field` | custom directive generation | Generates `s.Directive("format", new Dictionary<string, object?> { ["as"] = "ISO8601" })` on the `startsAt` sub-field lambda. No refusal — custom directives shipped in NGql 2.2. Notes that the directive name is schema-defined. |
| 19 | `I have one builder that declares a UserCard fragment and spreads it on viewer, and another that adds viewer.email — combine them` | Include + fragments | Uses `Include` on both builders under `MergingStrategy.MergeByFieldPath`. No refusal. Builds `viewer` with the lambda form in BOTH builders (not the dotted form in one), or warns about the `QueryMergeException` type conflict if the user's code mixes them. |
